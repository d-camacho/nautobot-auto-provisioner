...
# Variables may be overwritten in invoke.yml or by the environment variables INVOKE_NAUTOBOT_AUTO_PROVISIONER_xxx
namespace = Collection("nautobot_auto_provisioner")
namespace.configure(
    {
        "nautobot_auto_provisioner": {
            "nautobot_ver": "2.3.1",
            "project_name": "nautobot-auto-provisioner",
            "python_ver": "3.11",
            "local": False,
...
def _is_compose_included(context, name):
    return f"docker-compose.{name}.yml" in context.nautobot_auto_provisioner.compose_files
...
def docker_compose(context, command, **kwargs):
    ...
    build_env = {
        ...
        "COMPOSE_HTTP_TIMEOUT": context.nautobot_auto_provisioner.compose_http_timeout,
        "NAUTOBOT_VER": context.nautobot_auto_provisioner.nautobot_ver,
        "PYTHON_VER": context.nautobot_auto_provisioner.python_ver,
        **kwargs.pop("env", {}),
    }
    compose_command_tokens = [
        "docker compose",
        f"--project-name {context.nautobot_auto_provisioner.project_name}",
        f'--project-directory "{context.nautobot_auto_provisioner.compose_dir}"',
    ]

    for compose_file in context.nautobot_auto_provisioner.compose_files:
        compose_file_path = os.path.join(context.nautobot_auto_provisioner.compose_dir, compose_file)
...
def run_command(context, command, service="nautobot", **kwargs):
    ...
    if is_truthy(context.nautobot_auto_provisioner.local):
...
def build(context, force_rm=False, cache=True):
    ...
    print(f"Building Nautobot with Python {context.nautobot_auto_provisioner.python_ver}...")
...
def _ensure_creds_env_file(context):
    ...
    if not os.path.exists(
        os.path.join(context.nautobot_auto_provisioner.compose_dir, "creds.env")
    ):
        ...
        shutil.copy(
            os.path.join(
                context.nautobot_auto_provisioner.compose_dir, "creds.example.env"
            ),
            os.path.join(context.nautobot_auto_provisioner.compose_dir, "creds.env"),
        )
...
def _get_docker_nautobot_version(context, nautobot_ver=None, python_ver=None):
    ...
    if python_ver is None:
        python_ver = context.nautobot_auto_provisioner.python_ver
    dockerfile_path = os.path.join(context.nautobot_auto_provisioner.compose_dir, "Dockerfile")
...
def lock(context, check=False, constrain_nautobot_ver=False, constrain_python_ver=False):
    ...
    if constrain_nautobot_ver:
        ...
        if constrain_python_ver:
            command += f" --python {context.nautobot_auto_provisioner.python_ver}"
        try:
            ...
            command = f"poetry add --lock git+https://github.com/nautobot/nautobot.git#{context.nautobot_auto_provisioner.nautobot_ver}"
            if constrain_python_ver:
                command += f" --python {context.nautobot_auto_provisioner.python_ver}"
            run_command(context, command)
    else:
...
def makemigrations(context, name=""):
    ...
    command = "nautobot-server makemigrations nautobot_auto_provisioner"
...
@task(
    help={
        "version": "Version of Nautobot Auto Provisioner to generate the release notes for.",
    }
)
...
def pylint(context):
    ...
    command = f"{base_pylint_command} nautobot_auto_provisioner"
    if not run_command(context, command, warn=True):
        exit_code = 1

    # run the pylint_django migrations checkers on the migrations directory, if one exists
    migrations_dir = Path(__file__).absolute().parent / Path("nautobot_auto_provisioner") / Path("migrations")
    if migrations_dir.is_dir():
        migrations_pylint_command = (
            ...
            " nautobot_auto_provisioner.migrations"
        )
...
def unittest(  # noqa: PLR0913
    context,
    keepdb=False,
    label="nautobot_auto_provisioner",
    failfast=False,
    buffer=True,
    pattern="",
    verbose=False,
...
def tests(context, failfast=False, keepdb=False, lint_only=False):
    ...
    if not is_truthy(context.nautobot_auto_provisioner.local):
